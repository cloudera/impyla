...
from impala.udf import ship_udf, udf, FunctionContext, StringVal, IntVal, BigIntVal
...
signature = BigIntVal(FunctionContext, IntVal, BigIntVal, IntVal, BigIntVal,
	IntVal, BigIntVal, BigIntVal, BigIntVal, BigIntVal, BigIntVal, IntVal,
	BigIntVal, BigIntVal)

create_table_query = """
    CREATE EXTERNAL TABLE IF NOT EXISTS census_categ_text (age INT, workclass BIGINT,
...
for size in sizes:
    ...
    ship_udf(cursor, predict_income, '/user/laserson/test-udf/census_categ_%i.ll' % size, 'bottou01-10g.pa.cloudera.com', user='laserson', overwrite=True)
    ...
    print "impala_categ,%i,%i,%.2f,%.2f" % (size, len(udfs[size].func_code.co_code), end_score - start_score, end_compile - start_compile)
