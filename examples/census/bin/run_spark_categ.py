...
sc.addPyFile('/home/laserson/categ-models/model_0.py')
sc.addPyFile('/home/laserson/categ-models/model_100.py')
sc.addPyFile('/home/laserson/categ-models/model_500.py')
sc.addPyFile('/home/laserson/categ-models/model_1000.py')
sc.addPyFile('/home/laserson/categ-models/model_1500.py')
sc.addPyFile('/home/laserson/categ-models/model_2000.py')
...
def parse_obs(line):
    fields = line.split('\t')
    parsed = map(int, fields)
    # the None is where the FunctionContext would go; for Impala compatibility
    return [None] + parsed

observations = sc.textFile('/user/laserson/bigml/census_categ_text').map(parse_obs)
...
for size in sizes:
    ...
    end_score = time()
    results.append("spark_categ,%i,%i,%.2f," % (size, len(udfs[size].func_code.co_code), end_score - start_score))
