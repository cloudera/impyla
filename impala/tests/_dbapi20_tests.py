...
class DatabaseAPI20Test(unittest.TestCase):
    ...
    def test_Exceptions(self):
        ...
        # defined heirarchy.
        # under Python 3 StardardError no longer exists
        if sys.version[0] == '3':
    ...
    def test_description(self):
        ...
        try:
            ...
            self.assertEqual(len(cur.description[0]), 7,
                             'cursor.description[x] tuples must have 7 '
                             'elements'
                             )
    ...
    def test_rowcount(self):
        ...
        try:
            ...
            self.failUnless(cur.rowcount in (-1, 1),
                            'cursor.rowcount should == number or rows '
                            'inserted, or '
                            'set to -1 after executing an insert statement'
                            )
            cur.execute("select name from %sbooze" % self.table_prefix)
            self.failUnless(cur.rowcount in (-1, 1),
                            'cursor.rowcount should == number of rows '
                            'returned, or '
                            'set to -1 after executing a select statement'
                            )
            self.executeDDL2(cur)
            self.assertEqual(cur.rowcount, -1,
                             'cursor.rowcount not being reset to -1 after '
                             'executing '
    ...
    def test_close(self):
        ...
        # laserson note: the next to assertions are not clear to me from PEP
        #  249
    ...
    def test_executemany(self):
        ...
        try:
            ...
            self.assertEqual(len(res), 2,
                             'cursor.fetchall retrieved incorrect number of '
                             'rows'
                             )
            beers = sorted([res[0][0], res[1][0]])
    ...
    def test_fetchone(self):
        ...
        try:
            ...
            self.assertEqual(cur.fetchone(), None,
                             'cursor.fetchone should return None if no more '
                             'rows available'
                             )
            self.failUnless(cur.rowcount in (-1, 1))
        finally:
    ...
    def test_fetchmany(self):
        ...
        try:
            ...
            self.assertEqual(len(rows), 0,
                             'cursor.fetchmany should return an empty '
                             'sequence if '
                             'called after the whole result set has been '
                             'fetched'
                             )
            self.failUnless(cur.rowcount in (-1, 6))
