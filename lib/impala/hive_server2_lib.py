...
"""Python implementation of the thrift API to Impala through Hive Server 2"""
...
def create_impala_server_config(host='localhost', port=21050, principal='impala/%s' % socket.getfqdn()):
  config = {
      'server_name': 'impala',
      'server_host': host,
      'server_port': port,
...
class HiveServerClient(object):
  ...
  def __init__(self, query_server, user):
    self.query_server = query_server # config objet returned from create_impala_server_config()
    self.user = user
