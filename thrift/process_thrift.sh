...
cp $IMPALA_REPO/common/thrift/Types.thrift $IMPYLA_REPO/thrift
cp $IMPALA_REPO/thirdparty/thrift-*/contrib/fb303/if/fb303.thrift $IMPYLA_REPO/thrift
...
grep -v 'namespace py beeswaxd' $IMPALA_REPO/common/thrift/beeswax.thrift \
        > $IMPYLA_REPO/thrift/beeswax.thrift
...
cat $IMPALA_REPO/thirdparty/hive-*/src/metastore/if/hive_metastore.thrift \
        ...
        > $IMPYLA_REPO/thrift/hive_metastore.thrift
...
for THRIFT_FILE in $IMPYLA_REPO/thrift/*.thrift; do
    ...
    echo "    $BASE_NAME"
    cat $THRIFT_FILE | awk "$ADD_NAMESPACE_PY" > $IMPYLA_REPO/thrift/temp.thrift
    mv $IMPYLA_REPO/thrift/temp.thrift $THRIFT_FILE
done

echo "generating thrift python modules"
thrift -r --gen py:new_style -out $IMPYLA_REPO $IMPYLA_REPO/thrift/ImpalaService.thrift
